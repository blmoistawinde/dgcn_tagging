...
DATASET_DIR='./data'
FINE_GRAPH_DIR='./data/ontology_aser_pre_conj_wt_fid.pkl'
COARSE_GRAPH_DIR='./data/ontology_aser_pre_conj_wt_cid.pkl'
...
# Train
CUDA_VISIBLE_DEVICES=$GPU_ID python pytorch/main.py train --dataset_dir=$DATASET_DIR --graph_dir $FINE_GRAPH_DIR --workspace=$WORKSPACE --taxonomy_level=fine --model_type=$MODEL_TYPE --holdout_fold=1 --batch_size=$BATCH_SIZE --max_iters $MAX_ITERS --cuda
...
CUDA_VISIBLE_DEVICES=$GPU_ID python pytorch/main.py train --dataset_dir=$DATASET_DIR --graph_dir $COARSE_GRAPH_DIR --workspace=$WORKSPACE --taxonomy_level=coarse --model_type=$MODEL_TYPE --holdout_fold=1 --batch_size=$BATCH_SIZE --max_iters $MAX_ITERS --cuda

# CUDA_VISIBLE_DEVICES=$GPU_ID python pytorch/main.py inference_validation --dataset_dir=$DATASET_DIR --graph_dir $COARSE_GRAPH_DIR --workspace=$WORKSPACE --taxonomy_level=coarse --model_type=$MODEL_TYPE --holdout_fold=1 --iteration=5000 --batch_size=$BATCH_SIZE --cuda
