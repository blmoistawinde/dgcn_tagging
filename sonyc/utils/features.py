...
def read_metadata(metadata_path, data_type, mini_data, sample_portion=1.0):
    ...
    if sample_portion < 1.0:
        random_state = np.random.RandomState(1234)
        random_state.shuffle(audio_names)
        sample_num = int(sample_portion*len(audio_names))
        audio_names = audio_names[0 : sample_num]
    ...
    fine_label_nums = (fine_targets > 0).sum(0)
    coarse_label_nums = (coarse_targets > 0).sum(0)
    if sample_portion < 1:
        print(fine_label_nums)
        print(coarse_label_nums)
        with open(f"label_distrib_{sample_portion}.pkl", "wb") as f:
            pickle.dump([fine_label_nums, coarse_label_nums], f)
...
def calculate_feature_for_all_audio_files(args):
    ...
    mask_portion = args.mask_portion / 100.
    sample_portion = args.sample_portion / 100.
    ...
    if data_type in ['train', 'validate']:
        ...
        audios_dir = os.path.join(dataset_dir, 'test')
    ...
    if data_type in ['train', 'validate']:
        meta_dict = read_metadata(metadata_path, data_type, mini_data, sample_portion)
    elif data_type == 'evaluate':
    ...
    if 'fine_target' in meta_dict.keys():
        fine_targets = meta_dict['fine_target']
        if mask_portion > 0:
            if mask_portion == 1:
                fine_targets2 = np.zeros_like(fine_targets)
                mask_rows = np.arange(fine_targets.shape[0])
                mask_cols = fine_targets.argmax(1)
                fine_targets2[mask_rows, mask_cols] = fine_targets[mask_rows, mask_cols]
            else:
                row_indices = np.arange(fine_targets.shape[0])
                np.random.shuffle(row_indices)
                mask_rows = row_indices[int(mask_portion*fine_targets.shape[0])]
                # only take the most voted label per sample for masked rows
                mask_cols = fine_targets.argmax(1)[mask_rows]
                fine_targets2 = fine_targets.copy()
                fine_targets2[mask_rows, :] = 0.
                fine_targets2[mask_rows, mask_cols] = fine_targets[mask_rows, mask_cols]
            fine_targets = fine_targets2
        hf.create_dataset(
            name='fine_target', 
            data=fine_targets, 
            dtype=np.float32)
    ...
    if 'coarse_target' in meta_dict.keys():
        coarse_targets = meta_dict['coarse_target']
        if mask_portion > 0:
            if mask_portion == 1:
                coarse_targets2 = np.zeros_like(coarse_targets)
                mask_rows = np.arange(coarse_targets.shape[0])
                mask_cols = coarse_targets.argmax(1)
                coarse_targets2[mask_rows, mask_cols] = coarse_targets[mask_rows, mask_cols]
            else:
                row_indices = np.arange(coarse_targets.shape[0])
                np.random.shuffle(row_indices)
                mask_rows = row_indices[int(mask_portion*coarse_targets.shape[0])]
                # only take the most voted label per sample for masked rows
                mask_cols = coarse_targets.argmax(1)[mask_rows]
                coarse_targets2 = coarse_targets.copy()
                coarse_targets2[mask_rows, :] = 0.
                coarse_targets2[mask_rows, mask_cols] = coarse_targets[mask_rows, mask_cols]
            coarse_targets = coarse_targets2
        hf.create_dataset(
            name='coarse_target', 
            data=coarse_targets, 
            dtype=np.float32)
...
if __name__ == '__main__':
    ...
    parser_logmel.add_argument('--mask_portion', default=0., type=float, help='Portion of samples to mask to only one label.')
    parser_logmel.add_argument('--sample_portion', default=100., type=float, help='Portion of samples to use.')
