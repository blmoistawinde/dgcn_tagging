...
def plot(args):
    ...
    if len(args.graph_dir) > 0:
        graph_marker = args.graph_dir[args.graph_dir.rfind("/")+1:-4]
        save_out_path = 'results/{}_{}.pdf'.format(select, graph_marker)
    ...
    def _load_metrics(filename, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, data_type, model_type, loss_type, balanced, augmentation, batch_size, args=None):
        if "DoubleGCN" in model_type:
            graph_marker1 = args.graph_dir[args.graph_dir.rfind("/")+1:-4]
            graph_marker2 = args.graph_dir2[args.graph_dir2.rfind("/")+1:-4]
            model_marker = f"{model_type}_{graph_marker1}__{graph_marker2}"
        ...
        statistics_path = os.path.join(workspace, 'statistics', filename, 
            'sample_rate={},window_size={},hop_size={},mel_bins={},fmin={},fmax={}'.format(
            sample_rate, window_size, hop_size, mel_bins, fmin, fmax), 
            'data_type={}'.format(data_type), model_marker, 
            'loss_type={}'.format(loss_type), 'balanced={}'.format(balanced), 
            'augmentation={}'.format(augmentation), 'batch_size={}'.format(batch_size), 
            'statistics.pkl')
        ...
        test_map = np.array([statistics['average_precision'] for statistics in statistics_dict['test']])    # (N, classes_num)
        test_map = np.mean(test_map, axis=-1)
        # test_auc = np.array([statistics['auc'] for statistics in statistics_dict['test']])    # (N, classes_num)
        # test_auc = np.mean(test_auc, axis=-1)
        best_index = test_map.argmax()
        # best_index = test_auc.argmax()
        best_step = statistics_dict['test'][best_index]['iteration']
        mean_auc = statistics_dict['test'][best_index]['auc'].mean()
        dprime = d_prime(mean_auc)
        # best_MAP = statistics_dict['test'][best_index]['average_precision'].mean()
        best_MAP = test_map.max()
        print(model_marker)
        print(f"Get best MAP: {best_MAP:.5f}, Mean AUC: {mean_auc:.5f}, d-prime: {dprime:.5f}, at step {best_step}")
        legend = '{}, {}, bal={}, aug={}, bs={}'.format(data_type, model_marker, balanced, augmentation, batch_size)
    ...
    if select == '1_cnn13':
        ...
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_cnn10_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn10_16k', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn10,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_1_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_1', 'Cnn14_16k', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_5_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_5', 'Cnn14_16k', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_10_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_10', 'Cnn14_16k', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_1_gcn_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_1', 'Cnn14_16k_GCN', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_5_gcn_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_5', 'Cnn14_16k_GCN', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_10_gcn_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_10', 'Cnn14_16k_GCN', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_1_dgcn_inter_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_1', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_5_dgcn_inter_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_5', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_10_dgcn_inter_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train_10', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'none', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,none,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_cnn10_aug':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn10_16k', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn10,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_aug_gcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_GCN', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_cnn10_aug_gcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn10_16k_GCN', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn10_gcn,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_aug_dgcn2':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_DoubleGCN2', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn2,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_aug_dgcn_inter':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)
    
    elif select == '1_full_train_aug_gcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_GCN', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_cnn10_aug_gcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn10_16k_GCN', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn10_gcn,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_gcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_GCN', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,none,none', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_dgcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_DoubleGCN', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn,none,none', color='r', alpha=test_alpha)
        lines.append(line)
    elif select == '1_bal_train_dgcn2':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_DoubleGCN2', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn2,none,none', color='r', alpha=test_alpha)
        lines.append(line)
    elif select == '1_bal_train_dgcn_inter':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,none,none', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_dgcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_DoubleGCN', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn,none,none', color='r', alpha=test_alpha)
        lines.append(line)
    elif select == '1_full_train_dgcn2':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_DoubleGCN2', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn2,none,none', color='r', alpha=test_alpha)
        lines.append(line)
    elif select == '1_full_train_dgcn_inter':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,none,none', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train_aug_dgcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_DoubleGCN', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)
    elif select == '1_full_train_aug_dgcn2':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_DoubleGCN2', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn2,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)
    elif select == '1_full_train_aug_dgcn_inter':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_DoubleGCNInter', 'clip_bce', 'balanced', 'mixup', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_dgcn_inter,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,none,none', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_bal_train_aug_rgcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'balanced_train', 'Cnn14_16k_RGCN', 'clip_bce', 'balanced', 'mixup', 32, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_rgcn,balanced,mixup', color='r', alpha=test_alpha)
        lines.append(line)

    elif select == '1_full_train':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14,none,none', color='r', alpha=test_alpha)
        lines.append(line)
    
    elif select == '1_full_train_gcn':
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 512, 
            160, 64, 50, 8000, 'full_train', 'Cnn14_16k_GCN', 'clip_bce', 'none', 'none', 64, args)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        line, = ax.plot(test_map, label='cnn14_gcn,none,none', color='r', alpha=test_alpha)
        lines.append(line)
        ...
        (bal_map, test_map, legend) = _load_metrics('main', 16000, 2048, 
            320, 64, 50, 14000, 'full_train', 'Cnn14', 'clip_bce', 'balanced', 'mixup', 32)
        line, = ax.plot(bal_map, color='r', alpha=bal_alpha)
        ...
        (bal_map, test_map, legend) = _load_metrics0('main', 16000, 1024, 
            320, 64, 50, 14000, 'full_train', 'Cnn13', 'clip_bce', 'balanced', 'mixup', 32)
        line, = ax.plot(bal_map, color='b', alpha=bal_alpha)
...
def plot_class_iteration(args):
    ...
    iteration = 600000
    statistics_dict = _load_metrics('main', 16000, 1024, 
        320, 64, 50, 14000, 'full_train', 'Cnn13', 'clip_bce', 'balanced', 'mixup', 32, iteration)
...
def summary_stats(args):
    ...
    iteration = 600000
    output_dict['cnn13_system_iteration60k'] = _load_metrics('main', 16000, 1024, 
        320, 64, 50, 14000, 'full_train', 'Cnn13', 'clip_bce', 'balanced', 'mixup', 32, iteration)

    iteration = 560000
    output_dict['mobilenetv1_system_iteration56k'] = _load_metrics('main', 16000, 1024, 
        320, 64, 50, 14000, 'full_train', 'MobileNetV1', 'clip_bce', 'balanced', 'mixup', 32, iteration)
...
def plot_long_fig(args):
    ...
    maps_logmel_wavegram_cnn = _load_metrics0_classwise('main', 16000, 1024, 
        320, 64, 50, 14000, 'full_train', 'Cnn13_SpAndWav', 'clip_bce', 'balanced', 'mixup', 32)
...
def print_results(args):
    ...
    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn14', 'clip_bce', 'balanced', 'mixup', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn14_mixup_time_domain', 'clip_bce', 'balanced', 'mixup', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn14', 'clip_bce', 'balanced', 'none', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn14', 'clip_bce', 'none', 'none', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'balanced_train', 'Cnn14', 'clip_bce', 'none', 'none', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'balanced_train', 'Cnn14', 'clip_bce', 'balanced', 'none', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'balanced_train', 'Cnn14', 'clip_bce', 'balanced', 'mixup', 32)

    # 
    (mAP, mAUC, dprime) = _load_metrics0_classwise2('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn13_emb32', 'clip_bce', 'balanced', 'mixup', 32)

    (mAP, mAUC, dprime) = _load_metrics0_classwise2('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn13_emb128', 'clip_bce', 'balanced', 'mixup', 32)

    # partial
    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'partial_0.8_full_train', 'Cnn14', 'clip_bce', 'balanced', 'mixup', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'partial_0.5_full_train', 'Cnn14', 'clip_bce', 'balanced', 'mixup', 32)

    # Sample rate
    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn14_16k', 'clip_bce', 'balanced', 'mixup', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 64, 50, 14000, 'full_train', 'Cnn14_8k', 'clip_bce', 'balanced', 'mixup', 32)

    # Mel bins
    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 128, 50, 14000, 'full_train', 'Cnn14_mel128', 'clip_bce', 'balanced', 'mixup', 32)

    (mAP, mAUC, dprime) = _load_metrics_classwise('main', 16000, 1024, 320, 32, 50, 14000, 'full_train', 'Cnn14_mel32', 'clip_bce', 'balanced', 'mixup', 32)
...
if __name__ == '__main__':
    ...
    parser_plot.add_argument('--graph_dir', type=str, default="")
    parser_plot.add_argument('--graph_dir2', type=str, default="")
