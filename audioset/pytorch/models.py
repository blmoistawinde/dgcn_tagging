...
class GCNLabelEncoder(nn.Module):
    def __init__(self, graph: DGLGraph, emb_dims=[512, 1024, 2048], use_bias=True, num_hops=2, dropout=0.):
        super().__init__()
        self.graph = graph
        ...
        self.init_emb = nn.Parameter(torch.Tensor(self.num_nodes, emb_dims[0]))
        nn.init.kaiming_normal_(self.init_emb, a=math.sqrt(5))
        if num_hops == 1:
            self.gcn1 = GraphConv(emb_dims[0], emb_dims[-1], weight=True, bias=use_bias)
        elif num_hops == 2:
            self.gcn1 = GraphConv(emb_dims[0], emb_dims[1], weight=True, bias=use_bias)
            self.act1 = nn.LeakyReLU(0.2)
            self.drop1 = nn.Dropout(dropout)
            self.gcn2 = GraphConv(emb_dims[1], emb_dims[-1], weight=True, bias=use_bias)
    def forward(self):
        if self.num_hops == 1:
            x = self.gcn1(self.graph, self.init_emb, edge_weight=self.edge_weight)
        elif self.num_hops == 2:
            x = self.gcn1(self.graph, self.init_emb, edge_weight=self.edge_weight)
            x = self.act1(x)
            x = self.drop1(x)
            x = self.gcn2(self.graph, x, edge_weight=self.edge_weight)
        return x                             # [num_nodes, emb_dim]

class GCNLabelEncoder512(nn.Module):
    def __init__(self, graph: DGLGraph, emb_dim=512, use_bias=False, num_hops=2, dropout=0.):
        super().__init__()
        self.graph = graph
        if "_edge_weight" in graph.edata:
            self.edge_weight = nn.Parameter(torch.tensor(graph.edata["_edge_weight"], dtype=torch.float32), requires_grad=False)
...
class GCNLabelEncoderInter(nn.Module):
    def __init__(self, graph1: DGLGraph, graph2: DGLGraph, emb_dim=2048, use_bias=True, num_hops=2, dropout=0.):
        super().__init__()
        self.graph1 = graph1
        self.graph2 = graph2
        # if "_edge_weight" in graph.edata:
        ...
        self.num_hops = num_hops
        self.dropout = dropout
        self.init_emb = nn.Parameter(torch.Tensor(self.num_nodes, 512))
        nn.init.kaiming_normal_(self.init_emb, a=math.sqrt(5))
        if num_hops == 1:
            self.gcn_a1 = GraphConv(512, emb_dim, weight=True, bias=use_bias)
            self.gcn_b1 = GraphConv(512, emb_dim, weight=True, bias=use_bias)
        elif num_hops == 2:
            self.gcn_a1 = GraphConv(512, 1024, weight=True, bias=use_bias)
            self.act1 = nn.LeakyReLU(0.2)
            self.drop1 = nn.Dropout(dropout)
            self.gcn_a2 = GraphConv(1024, emb_dim, weight=True, bias=use_bias)
            self.gcn_b1 = GraphConv(512, 1024, weight=True, bias=use_bias)
            self.act2 = nn.LeakyReLU(0.2)
            self.drop2 = nn.Dropout(dropout)
            self.gcn_b2 = GraphConv(1024, emb_dim, weight=True, bias=use_bias)
    def forward(self):
        if self.num_hops == 1:
...
class GCNLabelEncoderInter512(nn.Module):
...
class RGCNLabelEncoder(nn.Module):
    def __init__(self, graph: DGLGraph, emb_dim=2048, use_bias=True, num_hops=2, dropout=0.):
        ...
        self.num_hops = num_hops
        self.dropout = dropout
        # roughly following AN ONTOLOGY-AWARE FRAMEWORK FOR AUDIO EVENT CLASSIFICATION
        # added another dropout
        self.init_emb = nn.Parameter(torch.Tensor(self.num_nodes, 512))
        nn.init.kaiming_normal_(self.init_emb, a=math.sqrt(5))   # very important, get bad result without this
        if num_hops == 1:
            self.gcn1 = RelGraphConv(512, emb_dim, self.num_rels, "bdd", bias=use_bias)
        elif num_hops == 2:
            self.gcn1 = RelGraphConv(512, 1024, self.num_rels, "bdd", bias=use_bias)
            self.act1 = nn.LeakyReLU(0.2)
            self.drop1 = nn.Dropout(dropout)
            self.gcn2 = RelGraphConv(1024, emb_dim, self.num_rels, "bdd", bias=use_bias)
    def forward(self):
        if self.num_hops == 1:
...
class Cnn6_GCN(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, graph, class_indices):
        
        super(Cnn6_GCN, self).__init__()
        ...
        self.graph_encoder = GCNLabelEncoder512(graph)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        clipwise_output = torch.sigmoid(embedding.mm(label_emb.T)) # (batch_size, num_classes)
...
class Cnn10_16k(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num):
        
        super(Cnn10_16k, self).__init__()
...
class Cnn10_16k_GCN(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, graph, class_indices):
        
        super(Cnn10_16k_GCN, self).__init__()
...
class Cnn10_16k_GCN2(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        fmax, classes_num, graph, class_indices):
        
        super(Cnn10_16k_GCN2, self).__init__()
        ...
        self.graph_encoder = GCNLabelEncoder512_2(graph)
        self.class_indices = class_indices
        # workaround to easily get a model's device
        self.dummy_param = nn.Parameter(torch.empty(0))
...
class Cnn14_16k(nn.Module):
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = self.conv_block6(x, pool_size=(1, 1), pool_type='avg')
        x = F.dropout(x, p=0.2, training=self.training)
        x = torch.mean(x, dim=3)  # (bs, channels, ~mel_bins)
...
class Cnn14_16k_GCN(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num, graph, class_indices):
        
        super(Cnn14_16k_GCN, self).__init__() 
...
class Cnn14_16k_RGCN(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num, graph, class_indices):
        
        super(Cnn14_16k_RGCN, self).__init__() 
...
class Cnn14_16k_DoubleGCN(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num, graph1, graph2, class_indices):
        
        super(Cnn14_16k_DoubleGCN, self).__init__() 
        ...
        self.graph_encoder1 = GCNLabelEncoder(graph1)
        self.graph_encoder2 = GCNLabelEncoder(graph2)
        self.class_indices = class_indices
        self.classes_num = len(class_indices)
        self.weight_fc = nn.Sequential(
            nn.Linear(2*self.classes_num, self.classes_num),
            nn.ReLU(),
            nn.Linear(self.classes_num, 1)
        )
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        x = x1 + x2               # (bs, channels)
        x = F.dropout(x, p=0.5, training=self.training)
        x1 = F.relu_(self.fc1(x))
        embedding1 = F.dropout(x1, p=0.5, training=self.training)
        x2 = F.relu_(self.fc2(x))
        embedding2 = F.dropout(x2, p=0.5, training=self.training)

        label_emb1 = self.graph_encoder1()   # (num_nodes, feature_maps)
        label_emb1 = label_emb1[self.class_indices]   # (num_classes, feature_maps)
        label_emb2 = self.graph_encoder2()   # (num_nodes, feature_maps)
        label_emb2 = label_emb2[self.class_indices]   # (num_classes, feature_maps)
        probs1 = torch.sigmoid(embedding1.mm(label_emb1.T))
        probs2 = torch.sigmoid(embedding2.mm(label_emb2.T))
        concat_probs = torch.cat([probs1, probs2], dim=1)
        weight = torch.sigmoid(self.weight_fc(concat_probs)) # (batch_size, 1)
        clipwise_output = weight * probs1 + (1-weight) * probs2  # (batch_size, num_classes)
        
        output_dict = {'clipwise_output': clipwise_output}

        return output_dict

class Cnn14_16k_DoubleGCN2(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num, graph1, graph2, class_indices):
        
        super(Cnn14_16k_DoubleGCN2, self).__init__() 
        ...
        self.graph_encoder1 = GCNLabelEncoder(graph1, emb_dim=1024)
        self.graph_encoder2 = GCNLabelEncoder(graph2, emb_dim=1024)
        self.class_indices = class_indices
        self.classes_num = len(class_indices)
        self.fusion_fc = nn.Linear(2048, 2048)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        label_emb2 = self.graph_encoder2()   # (num_nodes, feature_maps)
        label_emb2 = label_emb2[self.class_indices]   # (num_classes, feature_maps)
        concat_emb = torch.cat([label_emb1, label_emb2], dim=1)
        label_emb = self.fusion_fc(torch.tanh(concat_emb))
        # label_emb = torch.cat([label_emb1, label_emb2], dim=1)
        # label_emb = label_emb1 + label_emb2
        clipwise_output = torch.sigmoid(embedding.mm(label_emb.T)) # (batch_size, 1)
        
        output_dict = {'clipwise_output': clipwise_output}

        return output_dict

class Cnn14_16k_DoubleGCNInter(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num, graph1, graph2, class_indices):
        
        super(Cnn14_16k_DoubleGCNInter, self).__init__() 
        ...
        self.graph_encoder = GCNLabelEncoderInter(graph1, graph2, emb_dim=2048)
        self.class_indices = class_indices
        self.classes_num = len(class_indices)
    ...
    def forward(self, input, mixup_lambda=None):
        ...
        label_emb = self.graph_encoder()[self.class_indices]
        clipwise_output = torch.sigmoid(embedding.mm(label_emb.T)) # (batch_size, 1)
...
class Cnn10_16k_DoubleGCNInter(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, fmax, classes_num, graph1, graph2, class_indices):
        
        super(Cnn10_16k_DoubleGCNInter, self).__init__() 
        ...
        self.graph_encoder = GCNLabelEncoderInter512(graph1, graph2)
        self.class_indices = class_indices
        self.classes_num = len(class_indices)
